...
@app.route('/static/Nueva_tarea.css', methods=['GET'])
def static_files_nueva_tarea():
    """Ruta para servir archivos estáticos para tareas"""
    return app.send_static_file('Nueva_tarea.css')
...
def index():
    ...
    tareas_pendientes = db_manager.obtener_tareas(estado="Pendiente", to_dict=True)
    
    # LECTURA 2: Obtener la lista de proyectos para mostrar en la interfaz
    proyectos = db_manager.obtener_proyectos(to_dict=True)
